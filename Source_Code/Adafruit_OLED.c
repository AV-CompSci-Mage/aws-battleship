...
void writeCommand(unsigned char c) {
    ...
    // Set DC to low for command (pin 45)
...
//    GPIOPinWrite(GPIOA2_BASE, 0x2, 0x0);
    ...
    // fetch the data from the SPI bus (dummy data)
    unsigned long dummyData;
    SPIDataGet(GSPI_BASE, &dummyData);
...
void writeData(unsigned char c) {
    // Enable CS
    SPICSEnable(GSPI_BASE);
    // Set DC to high for data (pin 45)
    GPIOPinWrite(GPIOA3_BASE, 0x80, 0xFF);
    // Pull CS low
//    GPIOPinWrite(GPIOA2_BASE, 0x2, 0x0);
...
void Adafruit_Init(void){
  ...
  GPIOPinWrite(GPIOA3_BASE, 0x10, 0xFF);	// RESET = RESET_HIGH
